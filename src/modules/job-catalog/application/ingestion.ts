...
import { isCrawlable, type CrawlStatus, type SourceCompany } from "../domain/source";
import { canonicalizeJobUrl, slugifyTitle } from "../domain/urls";
import {
  updateCompanyAfterRun,
  type CompanyRunOutcome,
...
export async function runSourceCrawl(options: CrawlOptions): Promise<CrawlOutcome> {
  ...
  if (!isCrawlable(company)) {
    const reason = !company.active
      ? "company disabled"
      : company.crawlStatus === "paused"
        ? "source paused"
        : "crawl permission not recorded as allowed";
    logger.info({ event: "job_source_skipped", reason, source: company.slug });
    return {
      errorCount: 0,
      errorSummary: null,
      jobsDeactivated: 0,
  ...
  const locked = await withCompanyCrawlLock(company.id, () => runLockedSourceCrawl(options));
...
async function runLockedSourceCrawl(options: CrawlOptions): Promise<CrawlOutcome> {
  ...
  const connector = createConnectorForSource(company.sourceType);
  ...
  if (options.dryRun) {
    logger.info({ event: "job_crawl_dry_run_started", source: company.slug });
  }

  let runId: string | null = null;
  if (!options.dryRun) {
    try {
      runId = await withCrawlerRole((transaction) => startIngestionRun(transaction, company.id));
    } catch {
      return {
        errorCount: 1,
  ...
  if (fetchError) {
    ...
    const outcome: CompanyRunOutcome = {
      consecutiveFailures: company.consecutiveFailures + 1,
      crawlStatus: crawlStatusAfterFailure(
      ...
      nextCheckAt: nextCheckAtWithJitter(company.crawlFrequencyMinutes, now),
    };
    if (!options.dryRun) {
      await withCrawlerRole(async (transaction) => {
        ...
        await recordSourceEvent(transaction, company.id, "crawl_failed", code, {
          consecutiveFailures: outcome.consecutiveFailures,
        });
        if (code === "robots_blocked") {
          await recordSourceEvent(transaction, company.id, "robots_blocked", null, {});
        }
        if (outcome.crawlStatus === "paused") {
          await recordSourceEvent(transaction, company.id, "source_paused", "repeated failures", {
            threshold: configuration.failurePauseThreshold,
          });
        }
        await updateCompanyAfterRun(transaction, company.id, outcome);
      });
    }
    logger.warn({
      durationMs,
  ...
  try {
    const existing = await withCrawlerRole((transaction) =>
      listJobsForCompany(transaction, company.id),
    );
    ...
    if (!options.dryRun) {
      const applied = await withCrawlerRole((transaction) =>
        applyCrawlPlan(transaction, company.id, plan, {
          missingCrawlThreshold: configuration.missingCrawlThreshold,
          now,
    ...
    if (!options.dryRun) {
      await withCrawlerRole(async (transaction) => {
        ...
        if (discovered.length === 0) {
          await recordSourceEvent(transaction, company.id, "listing_empty", null, {});
        }
        await updateCompanyAfterRun(transaction, company.id, outcome);
      });
    }
    ...
    const durationMs = Date.now() - startedAt;
    const failureOutcome: CompanyRunOutcome = {
      consecutiveFailures: company.consecutiveFailures + 1,
      crawlStatus: crawlStatusAfterFailure(
        company.consecutiveFailures + 1,
        configuration.failurePauseThreshold,
    ...
    if (!options.dryRun && runId) {
      try {
        await withCrawlerRole(async (transaction) => {
          ...
          await recordSourceEvent(transaction, company.id, "crawl_failed", "database_error", {});
          await updateCompanyAfterRun(transaction, company.id, failureOutcome);
        });
      } catch {
        logger.error({ event: "job_source_failure_recording_failed", source: company.slug });
      }
    }
    logger.error({
      event: "job_source_crawl_failed",
...
function crawlStatusAfterFailure(failures: number, pauseThreshold: number): CrawlStatus {
  return failures >= pauseThreshold ? "paused" : "failing";
}
