...
export async function generateMetadata({ params }: { params: EmployerParams }): Promise<Metadata> {
  ...
  if (!employer) {
    const title = "Employer not found | OfferLab";
    return {
  ...
  const description = employerMetaDescription(employer);
  if (!employer.indexable) {
    const title = `${employer.name} | OfferLab`;
    return {
  ...
  const title = `${employer.name} | UK Employer Profile and Jobs | OfferLab`;
  return {
    alternates: { canonical: `/employers/${employer.slug}` },
    description,
    openGraph: { description, images: [], title, type: "website" },
          ...
          <section className="employer-profile-jobs" aria-labelledby="employer-jobs">
            ...
            {jobs.length === 0 ? (
              <p className="employer-profile-empty">
                {employer.name} has no roles currently listed through their official careers
                sources.
                {employer.imported_jobs > 0 && (
                  <>
                    {" "}
