...
- Answer Coach uses a provider-neutral typed interface. The first member-facing implementation is an explicitly labelled deterministic local rubric prototype. It sends no content to a provider, persists no review and never mutates the source answer.
